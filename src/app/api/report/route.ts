...
<head>
  ...
  <title>SmrtNetwork Report — ${generatedAt}</title>
</head>
<body style="margin: 0; padding: 0; background: #0f172a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
  <div style="max-width: 900px; margin: 0 auto; padding: 40px 24px;">
    ...
    <div style="margin-bottom: 32px; background: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 24px;">
      <h2 style="margin: 0 0 16px; font-size: 16px; font-weight: 600; color: #93c5fd; text-transform: uppercase; letter-spacing: 0.05em;">
        AI Health Narrative
      </h2>
      ${narrativeHtml}
    </div>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <thead>
            <tr style="border-bottom: 1px solid #334155; background: #0f172a;">
              <th style="padding: 10px 12px; text-align: left; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Network</th>
              <th style="padding: 10px 12px; text-align: left; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Types</th>
              <th style="padding: 10px 12px; text-align: center; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Total</th>
              <th style="padding: 10px 12px; text-align: center; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Online</th>
              <th style="padding: 10px 12px; text-align: center; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Offline</th>
              <th style="padding: 10px 12px; text-align: center; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Alerting</th>
              <th style="padding: 10px 12px; text-align: center; font-size: 12px; color: #64748b; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em;">Health</th>
            </tr>
          </thead>
          <tbody>
            ${rows}
...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json() as { orgId: string; sendEmail: boolean };
    const { orgId, sendEmail } = body;

    if (!orgId) {
      return NextResponse.json({ error: "orgId is required" }, { status: 400 });
    }

    // Fetch networks + device statuses
    const [networks, statuses] = await Promise.all([
      meraki.networks.list(orgId),
      meraki.devices.getStatuses(orgId),
    ]);

    // Build per-network summaries
    ...
    const networkLines = summaries
      .map(
        (s) =>
          `  - ${s.networkName}: ${s.total} devices, ${s.online} online, ${s.offline} offline, ${s.alerting} alerting, health ${s.healthScore}%`
      )
      .join("\n");

    const prompt = `You are a network operations analyst. Generate a concise health narrative for the following Meraki organization report.

Organization summary:
- ${networks.length} networks, ${totalDevices} total devices
- ${totalOnline} online, ${totalOffline} offline, ${totalAlerting} alerting
- Average health score: ${avgHealth}%

Per-network breakdown:
${networkLines}

Write a professional 2-4 paragraph summary covering:
1. Overall organization health
2. Networks that need immediate attention (if any)
...
Keep it factual and concise.`;
    ...
    try {
      ...
      narrative = `Organization health summary: ${totalOnline}/${totalDevices} devices online across ${networks.length} networks. Average health score: ${avgHealth}%.`;
    }

    const generatedAt = new Date().toLocaleString();
    const reportHtml = buildHtmlReport(summaries, narrative, generatedAt);
    ...
    if (sendEmail) {
      ...
      if (smtp.host && smtp.user && smtp.pass) {
        try {
          ...
          // Email send failed — still return the report
          emailSent = false;
        }
      }
    }
